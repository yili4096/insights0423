    ...
    angular.module('territoryApp', ['ngRoute', 'ngSanitize', 'ui.bootstrap'])
        ...
        function AppConfig($routeProvider, $compileProvider, $locationProvider, VEEVA_MESSAGES) {
            ...
            $routeProvider
                ...
                .when('/approvedEmailActivity', {
                    templateUrl: 'views/approved-email-activity.html',
                    controller: 'ApprovedEmailActivityController',
                    controllerAs: 'ae'
                });
        }
})();
...
(function() {
    ...
    angular.module('territoryApp')
        .controller('ApprovedEmailActivityController', ['$scope', '$timeout', '$q', 'userService', 'sentEmailService', 'accountService', 'territoryService',
            'approvedDocumentService', 'productService', 'reportUtil', 'messageService', 'labelService', ApprovedEmailController]);

    function ApprovedEmailController($scope, $timeout, $q, userService, sentEmailService, accountService, territoryService,
                                     approvedDocumentService, productService, reportUtil, messageService, labelService) {
        var ae = this;
        ...
        ae.frequencyChartMode = 0;

        // Sent document table storage
        ae.sentDocumentBySentCountTables = [{
            type: messageService.getMessage('template')
        }, {
            type: messageService.getMessage('fragment')
        }];
        ae.sentDocumentByOpenRateTables = [{
            type: messageService.getMessage('template')
        }, {
            type: messageService.getMessage('fragment')
        }];
        ae.sentDocumentBySentCountTableMode = 0;
        ae.sentDocumentByOpenRateTableMode = 0;

        // Static data
        var maxSentEmailFrequency = 4;
        var currentUser = userService.getCurrentUser();
        var labels = labelService.labels;

        // Promises
        var sentEmailDataDeferred = $q.defer();
        var accountDataDeferred = $q.defer();
        var sentDocumentDataDeferred = $q.defer();
        var mySetupProductDataDeferred = $q.defer();

        var mainDataPromises = [
            sentEmailDataDeferred.promise,
            accountDataDeferred.promise
        ];
        var allPromises = [
            sentEmailDataDeferred.promise,
            accountDataDeferred.promise,
            sentDocumentDataDeferred.promise,
        ...
        initiateSentEmailFrequencyTableData();
        initiateAccountFrequencyTableData();
        initiateAccountDetailTableData();

        sentEmailService.fetchSentEmailData().then(function(resp) {
            ae.sentEmails = resp.data;
            sentEmailDataDeferred.resolve();
        }, function(resp) {
            sentEmailDataDeferred.reject(resp);
        });
        ...
        $q.all(mainDataPromises).then(function() {
            fetchApprovedDocuments().then(function() {
                initiateSentDocumentFrequencyTableData();
                updateSentDocumentFrequencyTableData();
                sentDocumentDataDeferred.resolve();
            });

            updateProductFrequencyTableData();
            updateDailyFrequencyChartData();
            updateWeeklyFrequencyChartData();
            updateSentEmailFrequencyTableData();
            updateAccountFrequencyData();
            updateAccountDetailData(true);
            ae.mainDataReady = true;
        });
        ...
        ae.sentEmailDataUpdate = function(sentEmails) {
            ae.sentEmails = sentEmails;
            updateProductFrequencyTableData();
            updateDailyFrequencyChartData();
            updateWeeklyFrequencyChartData();
            updateSentEmailFrequencyTableData();
        ...
        ae.accountDataUpdate = function(accounts) {
            ae.accounts = accounts;
            updateProductFrequencyTableData();
            updateDailyFrequencyChartData();
            updateWeeklyFrequencyChartData();
            updateSentEmailFrequencyTableData();
        ...
        ae.countAllAccounts = function() {
            return ae.accounts.length;
        };

        ae.countAccountsWithSuccessfulSentEmail = function() {
            return getAccountsWithSuccessfulSentEmail().length;
        };

        ae.getAverageSentEmailPerAccount = function() {
            var totalAccountsWithSuccessfulSentEmail = _.size(getAccountsWithSuccessfulSentEmail());
            if(totalAccountsWithSuccessfulSentEmail === 0) {
                return 0;
            }

            var totalSuccessfullySentEmailFromFilteredAccount = _.reduce(groupSuccessfulSentEmailsByFilteredAccount(),function(memo, sentEmails) {
                return memo + sentEmails.length;
            }, 0);

            return Math.round(10*totalSuccessfullySentEmailFromFilteredAccount / totalAccountsWithSuccessfulSentEmail)/10;
        };

        ae.countOpenedSentEmailFromFilteredAccount = function() {
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();
            var openedSentEmail = 0;

            angular.forEach(successfulSentEmailsByFilteredAccount, function(sentEmails) {
                angular.forEach(sentEmails,function(sentEmail) {
                    if(isEmailOpened(sentEmail)) {
                        openedSentEmail += 1;
                    }
                });
            });

            return openedSentEmail;
        };

        ae.countAllSuccessfulSentEmailsFromFilteredAccount = function() {
            return _.reduce(groupSuccessfulSentEmailsByFilteredAccount(),function(memo, sentEmails) {
                return memo + sentEmails.length;
            }, 0);
        };

        ae.goToAccountDetailView = function(rowNumber) {
            if(rowNumber >= maxSentEmailFrequency) {
                ae.preSelectedFrequency = rowNumber + '+';
            } else {
                ae.preSelectedFrequency = rowNumber;
            }
        ...
        ae.getSentEmailFrequencyList = function() {
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();
            var sentEmailFrequencyList = [];
            var tracker = {};

            angular.forEach(successfulSentEmailsByFilteredAccount, function(sentEmails) {
                var sentEmailCount = sentEmails.length;
                if(sentEmailCount >= maxSentEmailFrequency) {
                    sentEmailCount = maxSentEmailFrequency + '+';
                }
                if(!tracker[sentEmailCount]) {
                    sentEmailFrequencyList.push(sentEmailCount);
                    tracker[sentEmailCount] = true;
                }
            });

            return _.sortBy(sentEmailFrequencyList, function(frequency) {
                return parseInt(frequency);
            });
        };

        ae.sentEmailFrequencyUpdate = function(frequency) {
            if(angular.isDefined(frequency.value)) {
                ae.accountDetailTable.filter.frequency = frequency.value;
            } else {
                ae.accountDetailTable.filter.frequency = null;
            }
        };

        ae.accountDetailTableFilter = function(row) {
            if(angular.isNumber(ae.accountDetailTable.filter.frequency)) {
                return row[labels.emails.value] === ae.accountDetailTable.filter.frequency;

            } else if(angular.isString(ae.accountDetailTable.filter.frequency)) {
                // Group accounts together for sent email frequency equal or larger than the maxSentEmailFrequency
                if(ae.accountDetailTable.filter.frequency === maxSentEmailFrequency + '+') {
                    return row[labels.emails.value] >= maxSentEmailFrequency;
                }
            }
        ...
        ae.switchFrequencyChart = function(index) {
            ae.frequencyChartMode = index;
        ...
        ae.getLabelTranslation = function(key) {
            if(labels[key]) {
         ...
         * Private Functions
         */
        function fetchApprovedDocuments() {
            var deferred = $q.defer();
            var templateIds = getPropertyFromSuccessfulSentEmailsWithinFilteredAccount('Approved_Email_Template_vod__c');
            var fragmentIds = getPropertyFromSuccessfulSentEmailsWithinFilteredAccount('Email_Fragments_vod__c', ',');

            approvedDocumentService.fetchApprovedDocuments(templateIds.concat(fragmentIds)).then(function() {
                deferred.resolve();
            });
        ...
        function initiateProductFrequencyTableData() {
            ae.productFrequencyTable = {
                headers: [labels.product, labels.emails],
                data: []
            };
        }

        function initiateSentEmailFrequencyTableData() {
            ae.sentEmailFrequencyTable = {
        ...
        function initiateAccountFrequencyTableData() {
            ae.topAccountTable = {
                headers: [labels.accountName, labels.emails, labels.openPercentage],
                data: []
            };

            ae.leastAccountTable = {
                headers: [labels.accountName, labels.emails, labels.openPercentage],
                data: []
            };
        }
        ...
        function initiateAccountDetailTableData() {
            ae.accountDetailTable = {
                headers: [labels.accountName, labels.emails, labels.openPercentage, labels.opened],
                data: [],
                filter: {}
            };
        }

        function initiateSentDocumentFrequencyTableData() {
            ae.sentDocumentBySentCountTables[0].headers = [labels.emailTemplateName, labels.emails, labels.openPercentage];
            ae.sentDocumentBySentCountTables[1].headers = [labels.emailFragmentName, labels.emails, labels.openPercentage];
            ae.sentDocumentByOpenRateTables[0].headers = [labels.emailTemplateName, labels.emails, labels.openPercentage];
            ae.sentDocumentByOpenRateTables[1].headers = [labels.emailFragmentName, labels.emails, labels.openPercentage];

            // Associate template name and fragment name on sent email
            angular.forEach(ae.sentEmails, function(sentEmail) {
                if(sentEmail.Approved_Email_Template_vod__c && angular.isString(sentEmail.Approved_Email_Template_vod__c.value) && sentEmail.Approved_Email_Template_vod__c.value.length) {
                    var templateId = sentEmail.Approved_Email_Template_vod__c.value;
                    var foundTemplate = approvedDocumentService.findApprovedDocumentById(templateId);
                    if(foundTemplate && foundTemplate.Name) {
                        sentEmail.emailTemplateName = foundTemplate.Name.display;
                    }
                }

                if(sentEmail.Email_Fragments_vod__c && angular.isString(sentEmail.Email_Fragments_vod__c.value) && sentEmail.Email_Fragments_vod__c.value.length) {
                    var fragmentIds = sentEmail.Email_Fragments_vod__c.value;
                    var fragmentIdList = fragmentIds.split(',');
                    var fragmentList = [];

                    angular.forEach(fragmentIdList, function(fragmentId) {
                        var foundFragment = approvedDocumentService.findApprovedDocumentById(fragmentId);
                        if(foundFragment && foundFragment.Id && foundFragment.Name) {
                            fragmentList.push({
                                id: foundFragment.Id.value,
                                name: foundFragment.Name.display
                            });
                        }
                    });

                    sentEmail.emailFragments = fragmentList;
                }
            });
        }
        ...
        function updateProductFrequencyTableData() {
            var newData = [];
            var sentEmailsByProductId = groupSuccessfulSentEmailsForFilteredAccountByProductId();

            angular.forEach(sentEmailsByProductId, function(sentEmails) {
                if(angular.isArray(sentEmails) && sentEmails[0].productName) {
                    var row = {};
                    row[labels.product.value] = sentEmails[0].productName.display;
                    row[labels.emails.value] = sentEmails.length;
                    newData.push(row);
                }
            });
            ...
            newData = _.sortBy(newData, function(row) {
                return row[labels.emails.value] * -1;
            });

            var maxProduct = 3;
            if(newData.length >= maxProduct) {
                ae.productFrequencyTable.data = newData.slice(0, maxProduct);
            } else {
                ae.productFrequencyTable.data = newData;
            }
        }

        function updateDailyFrequencyChartData() {
            var sentEmails = getSuccessfulSentEmailsFromFilteredAccount();

            var totalDays = reportUtil.countDaysInBetween(sentEmailService.getStartDate(), sentEmailService.getEndDate());
            ae.avgSentEmailsPerDay = totalDays === 0 ? 0 : Math.round(10 * sentEmails.length / totalDays)/10;

            var newChartData = reportUtil.generateDailyFrequencyChartData(sentEmails, 'Email_Sent_Date_vod__c', ae.avgSentEmailsPerDay);
            ae.frequencyChart[0] = _.extendOwn(ae.frequencyChart[0], newChartData);
        }
        ...
        function updateWeeklyFrequencyChartData() {
            ...
            var newChartData = reportUtil.generateWeeklyFrequencyChartData(sentEmails, 'Email_Sent_Date_vod__c', ae.avgSentEmailsPerDay);
            ae.frequencyChart[1] = _.extendOwn(ae.frequencyChart[1], newChartData);
        }

        function updateSentEmailFrequencyTableData() {
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();
            var accountIdsBySentEmailFrequency = groupAccountIdsBySentEmailFrequency(successfulSentEmailsByFilteredAccount);
            var newData = [];

            for(var i = 0; i <= maxSentEmailFrequency; i++) {
                var row = {};
                var sentEmailFrequency = i;
                var accountCount = 0;
                var accountPercentage = 0;

                // Count account together if it has maxSentEmailFrequency of sent emails or more
                if(sentEmailFrequency >= maxSentEmailFrequency) {
                    angular.forEach(successfulSentEmailsByFilteredAccount, function(sentEmails) {
                        if(sentEmails.length >= maxSentEmailFrequency) {
                            accountCount += 1;
                        }
                    });
                    sentEmailFrequency = maxSentEmailFrequency + '+';
                } else {
                    accountCount = _.size(accountIdsBySentEmailFrequency[sentEmailFrequency]);
                }

                if(ae.accounts.length > 0) {
                    accountPercentage = Math.floor((accountCount/ae.accounts.length) * 100);
                }

                row[labels.frequency.value] = sentEmailFrequency;
                row[labels.accountCount.value] = accountCount;
                row[labels.accountPercentage.value] = accountPercentage + '%';
                newData.push(row);
            }

            ae.sentEmailFrequencyTable.data = newData;
        }

        function updateSentDocumentFrequencyTableData() {
            var sentEmailList = getSuccessfulSentEmailsFromFilteredAccount();

            // Group the sent emails by template id
            var sentEmailsByTemplate = _.groupBy(sentEmailList, function (sentEmail) {
                if (sentEmail.Approved_Email_Template_vod__c) {
                    return sentEmail.Approved_Email_Template_vod__c.value;
                }
            });

            // Group the sent emails by fragment
            var fragments = {};
            angular.forEach(sentEmailList, function(sentEmail) {
                angular.forEach(sentEmail.emailFragments, function(fragment) {
                    if(!fragments[fragment.id]) {
                        ...
                        fragments[fragment.id].frequency += 1;
                        fragments[fragment.id].sentEmails.push(sentEmail);
                    }
                });
            });

            // Sort the template - sent emails group list by sent count
            var sortedTemplateSentEmailsBySentCount = _.sortBy(sentEmailsByTemplate, function (sentEmails) {
                return sentEmails.length;
            });
            sortedTemplateSentEmailsBySentCount.reverse();

            // Sort the template - sent emails group list by open rate
            var sortedTemplateSentEmailsByOpenRate = _.sortBy(sentEmailsByTemplate, function(sentEmails) {
                return calculateEmailOpenedPercentage(sentEmails);
            });
            sortedTemplateSentEmailsByOpenRate.reverse();

            // Sort the fragment list by (email)sent count
            var sortedFragmentsBySentCount = _.sortBy(fragments, function (fragment) {
                return fragment.sentEmails.length;
            });
            sortedFragmentsBySentCount.reverse();

            // Sort the fragment list by (email)open rate
            var sortedFragmentsByOpenRate = _.sortBy(fragments, function(fragment) {
                return calculateEmailOpenedPercentage(fragment.sentEmails);
            });
            sortedFragmentsByOpenRate.reverse();

            // Begin to fill in new table data
            var maxRowsPerTable = 3;

            var newTemplateDataBySentCount = [];
            for(var i = 0; i < maxRowsPerTable && sortedTemplateSentEmailsBySentCount[i]; i++) {
                var sentEmailsForTemplate = sortedTemplateSentEmailsBySentCount[i];
                if(sentEmailsForTemplate[0] && sentEmailsForTemplate[0].emailTemplateName) {
                    var row = {};
                    row[labels.emailTemplateName.value] = sentEmailsForTemplate[0].emailTemplateName;
                    row[labels.emails.value] = sentEmailsForTemplate.length;
                    row[labels.openPercentage.value] = calculateEmailOpenedPercentage(sentEmailsForTemplate) + '%';
                    newTemplateDataBySentCount.push(row);
                }
            }

            var newTemplateDataByOpenRate = [];
            for(var x = 0; x < maxRowsPerTable && sortedTemplateSentEmailsByOpenRate[x]; x++) {
                var sentEmailsForTemplate = sortedTemplateSentEmailsByOpenRate[x];
                if(sentEmailsForTemplate[0] && sentEmailsForTemplate[0].emailTemplateName) {
                    var row = {};
                    row[labels.emailTemplateName.value] = sentEmailsForTemplate[0].emailTemplateName;
                    row[labels.emails.value] = sentEmailsForTemplate.length;
                    row[labels.openPercentage.value] = calculateEmailOpenedPercentage(sentEmailsForTemplate) + '%';
                    newTemplateDataByOpenRate.push(row);
                }
            }

            var newFragmentDataBySentCount = [];
            for(var j = 0; j < maxRowsPerTable && sortedFragmentsBySentCount[j]; j++) {
                var fragment = sortedFragmentsBySentCount[j];
                var row = {};
                row[labels.emailFragmentName.value] = fragment.name;
                row[labels.emails.value] = fragment.sentEmails.length;
                row[labels.openPercentage.value] = calculateEmailOpenedPercentage(fragment.sentEmails) + '%';;
                newFragmentDataBySentCount.push(row);
            }

            var newFragmentDataByOpenRate = [];
            for(var y = 0; y < maxRowsPerTable && sortedFragmentsByOpenRate[y]; y++) {
                var fragment = sortedFragmentsByOpenRate[y];
                var row = {};
                row[labels.emailFragmentName.value] = fragment.name;
                row[labels.emails.value] = fragment.sentEmails.length;
                row[labels.openPercentage.value] = calculateEmailOpenedPercentage(fragment.sentEmails) + '%';;
                newFragmentDataByOpenRate.push(row);
            }

            ae.sentDocumentBySentCountTables[0].data = newTemplateDataBySentCount;
            ae.sentDocumentBySentCountTables[1].data = newFragmentDataBySentCount;

            ae.sentDocumentByOpenRateTables[0].data = newTemplateDataByOpenRate;
            ae.sentDocumentByOpenRateTables[1].data = newFragmentDataByOpenRate;
        }

        function updateAccountFrequencyData() {
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();
            var sortedSentEmailsByFrequency = _.sortBy(successfulSentEmailsByFilteredAccount, function(sentEmails) {
                return sentEmails.length * -1;
            });
            // Skip account with no sent email
            sortedSentEmailsByFrequency = _.filter(sortedSentEmailsByFrequency, function(sentEmails) {
            ...
            for(var i = 0; i < maxRowsPerTable && sortedSentEmailsByFrequency[i]; i++) {
                // Since list 'sortedSentEmailsByFrequency' is comes from object 'callsByFilteredAccount', which means
                ...
                if(sortedSentEmailsByFrequency[i][0].Account_vod__c) {
                    var accountId = sortedSentEmailsByFrequency[i][0].Account_vod__c.value;
                    var account = _.find(ae.accounts, function(account) {
                        return account.Id.value === accountId;
                    });
                    var accountName = reportUtil.formatAccountName(account);
                    var emailCount = sortedSentEmailsByFrequency[i].length;
                    var row = {};

                    row[labels.accountId.value] = accountId;
                    row[labels.accountName.value] = accountName;
                    row[labels.emails.value] = emailCount;
                    row[labels.openPercentage.value] = calculateEmailOpenedPercentage(sortedSentEmailsByFrequency[i]) + '%';
            ...
            if(sortedSentEmailsByFrequency[walker - 1]) {
                leastFrequencyFromTopAccount = sortedSentEmailsByFrequency[walker - 1].length;
            }
            var lastIndex = sortedSentEmailsByFrequency.length - 1;
            ...
            if(lastIndex >= walker) {
                for(var i = 0; i < maxRowsPerTable && (lastIndex - i) >= walker; i++) {
                    ...
                    var emailCount = sortedSentEmailsByFrequency[lastIndex - i].length;
                    if(emailCount === leastFrequencyFromTopAccount) {
                        break;
                    }

                    if(sortedSentEmailsByFrequency[lastIndex - i][0].Account_vod__c) {
                        var accountId = sortedSentEmailsByFrequency[lastIndex - i][0].Account_vod__c.value;
                        var account = _.find(ae.accounts, function(account) {
                            return account.Id.value === accountId;
                        });
                        var accountName = reportUtil.formatAccountName(account);
                        ...
                        row[labels.accountName.value] = accountName;
                        row[labels.emails.value] = emailCount;
                        row[labels.openPercentage.value] = calculateEmailOpenedPercentage(sortedSentEmailsByFrequency[lastIndex - i]) + '%';
                        newLeastAccountData.push(row);
                    }
                }
            }

            ae.topAccountTable.data = newTopAccountData;
            ae.leastAccountTable.data = newLeastAccountData;
        }

        function updateAccountDetailData(sentEmailDataRefreshed) {
            var accountDetailData = [];
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();

            angular.forEach(successfulSentEmailsByFilteredAccount, function(sentEmails, accountId) {
                var row = {};

                // Re-calculate call related data
                if(sentEmailDataRefreshed) {
                    row[labels.emails.value] = sentEmails.length;
                    row[labels.openPercentage.value] = calculateEmailOpenedPercentage(sentEmails) + '%';
                    row[labels.opened.value] = countOpenedEmail(sentEmails);
                }
                // get call related data from the existed table data
                else {
                    var existedRow = _.find(ae.accountDetailTable.data, function(tableRow) {
                        return tableRow[labels.accountId.value] === accountId;
                    });

                    if(existedRow) {
                        row[labels.emails.value] = existedRow[labels.emails.value];
                        row[labels.openPercentage.value] = existedRow[labels.openPercentage.value] + '%';
                        row[labels.opened.value] = existedRow[labels.opened.value];
                    }
                }
            ...
            ae.accountDetailTable.data = accountDetailData;
        }

        /**
         * Group successfully sent emails by account
         * Sent emails might be filtered by the date range picker
         * @returns {*|Object}
         */
        function groupSuccessfulSentEmailsByAccount() {
            return _.groupBy(ae.sentEmails, function(sentEmail) {
                if(isEmailSentSuccessfully(sentEmail) && sentEmail.Account_vod__c) {
                    return sentEmail.Account_vod__c.value;
                }
            });
        }

        /**
         * Group sent emails that has been sent successfully by filtered account
         * Build on top of groupSuccessfulSentEmailsByAccount()
         * Accounts might be filtered by the territory filter
         * Filtered account with 0 sent email will be add in
         * @returns {{}}
         */
        function groupSuccessfulSentEmailsByFilteredAccount() {
            var successfullySentEmailsByAccount = groupSuccessfulSentEmailsByAccount();
            var successfullySentEmailsByFilteredAccount = {};

            angular.forEach(ae.accounts, function(account) {
                if(account.Id) {
                    var accountId = account.Id.value;
                    if(successfullySentEmailsByAccount[accountId]) {
                        successfullySentEmailsByFilteredAccount[accountId] = successfullySentEmailsByAccount[accountId];
                    } else {
                        successfullySentEmailsByFilteredAccount[accountId] = [];
                    }
                }
            });

            return successfullySentEmailsByFilteredAccount;
        }

        /**
         * Group successful sent emails from the filtered account by product id
         * @returns {*|Object}
         */
        function groupSuccessfulSentEmailsForFilteredAccountByProductId() {
            var successfulSentEmailsForFilteredAccount = [];

            angular.forEach(groupSuccessfulSentEmailsByFilteredAccount(), function(sentEmails) {
                if(sentEmails.length > 0) {
                    successfulSentEmailsForFilteredAccount = successfulSentEmailsForFilteredAccount.concat(sentEmails);
                }
            });

            return _.groupBy(successfulSentEmailsForFilteredAccount, function(sentEmail) {
                if(sentEmail.Product_vod__c) {
                    return sentEmail.Product_vod__c.value;
                }
            });
        }

        /**
         * Get a list of account that has successfully received at least one email
         * @returns {Array}
         */
        function getAccountsWithSuccessfulSentEmail() {
            var accounts = [];
            var tracker = {};

            angular.forEach(ae.sentEmails, function(sentEmail) {
                if(isEmailSentSuccessfully(sentEmail) && sentEmail.Account_vod__c) {
                    var accountId = sentEmail.Account_vod__c.value;

                    if(!tracker[accountId]) {
                        tracker[accountId] = true;
                        var account = _.find(ae.accounts, function(anAccount) {
                            if(anAccount.Id) {
                                return anAccount.Id.value === accountId;
                            } else {
                                return false;
                            }
                        });
        ...
        function getPropertyFromSuccessfulSentEmailsWithinFilteredAccount(property, valueDelimiter) {
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();
            var result = [];
            var tracker = {};
            ...
            angular.forEach(successfulSentEmailsByFilteredAccount, function(sentEmails) {
                angular.forEach(sentEmails, function(sentEmail) {
                    if(sentEmail[property]) {
                        var propertyValue = sentEmail[property].value;

                        if(angular.isString(valueDelimiter)) {
                            var values = propertyValue.split(valueDelimiter);
                        ...
                        } else if(!tracker[propertyValue]) {
                            result.push(propertyValue);
                            tracker[propertyValue] = true;
                        }
                    }
                });
            });
        ...
        /**
         * Group account ids by sent email frequency(number)
         * @param sentEmailsByFilteredAccount - a map return from groupSuccessfulSentEmailsByFilteredAccount()
         * @returns {{}}
         */
        function groupAccountIdsBySentEmailFrequency(sentEmailsByFilteredAccount) {
            var accountIdsBySentEmailFrequency = {};

            angular.forEach(sentEmailsByFilteredAccount, function(sentEmails, accountId) {
                var sentEmailFrequency = sentEmails.length;
                if(!accountIdsBySentEmailFrequency[sentEmailFrequency]) {
                    accountIdsBySentEmailFrequency[sentEmailFrequency] = [];
                }
                accountIdsBySentEmailFrequency[sentEmailFrequency].push(accountId);
            });

            return accountIdsBySentEmailFrequency;
        }

        /**
         * Determine if the given sentEmail object has successful sent status
         * @param sentEmail - sent email object
         * @returns {boolean}
         */
        function isEmailSentSuccessfully(sentEmail) {
            if(sentEmail.Status_vod__c) {
                var status = sentEmail.Status_vod__c.value;
                return status === 'Sent_vod' || status === 'Delivered_vod' || status === 'Unsubscribed_vod';
            }
        ...
        /**
         * Determine if the given sentEmail object has the opened flag set to true(1)
         * @param sentEmail - sent email object
         * @returns {*}
         */
        function isEmailOpened(sentEmail) {
            if(veevaUtil.isWin8()) {
                return isEmailSentSuccessfully(sentEmail) && sentEmail.Opened_vod__c.value === '1';
            } else if(sentEmail.Opened_vod__c) {
                return isEmailSentSuccessfully(sentEmail) && sentEmail.Opened_vod__c.value;
            }
            return false;
        }

        /**
         * Count the total amount of opened email from the given sent email list
         * @param sentEmails - Array of sent email
         * @returns {number}
         */
        function countOpenedEmail(sentEmails) {
            var count = 0;
        ...
        /**
         * Calculate email opened percentage from the given sent email list
         * @param sentEmails - Array of sent email
         * @returns {number}
         */
        function calculateEmailOpenedPercentage(sentEmails) {
            var totalEmails = sentEmails.length;

            if(totalEmails) {
                var totalOpenedEmails = countOpenedEmail(sentEmails);
                return Math.round(100 * totalOpenedEmails / totalEmails);
            } else {
                return 0;
            }
        }
        ...
        /**
         * Extract a list of sent emails from all the (successful)sent emails under current filtered account
         * @returns {Array} - An array of sent emails
         */
        function getSuccessfulSentEmailsFromFilteredAccount() {
            var sentEmails = [];
            var successfulSentEmailsByFilteredAccount = groupSuccessfulSentEmailsByFilteredAccount();

            angular.forEach(successfulSentEmailsByFilteredAccount, function(emails) {
                if(emails.length > 0) {
...
(function() {
    ...
    function CallActivityController($scope, $timeout, $q, userService, accountService, callService, callDetailService,
        ...
        function updateWeeklyFrequencyChartData() {
            ...
            var newChartData = reportUtil.generateWeeklyFrequencyChartData(calls, 'Call_Date_vod__c', call.avgCallsPerDay);
...
angular.module('territoryApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/approved-email-activity-filter.html',
  ...
  $templateCache.put('views/approved-email-activity.html',
    "<div ng-show=\"ae.mainView\">\n" +
    "    <approved-email-activity-filter ng-if=\"ae.allReady\" sent-email-data-update=\"ae.sentEmailDataUpdate(sentEmails)\" account-data-update=\"ae.accountDataUpdate(accounts)\"></approved-email-activity-filter>\n" +
    "\n" +
    "    <div class=\"ae-summary-section-1\">\n" +
    "        <div class=\"table structure-table\">\n" +
    "            <div class=\"cell half section-container\">\n" +
    "                <div class=\"section-title\">{{'emailReach' | message}}</div>\n" +
    "                <div class=\"section-body inline-block-container\" ng-if=\"ae.mainDataReady\">\n" +
    "                    <div class=\"align-top\">\n" +
    "                        <doughnut-chart ng-style=\"ae.getDoughnutStyle()\" color1=\"#ff5575\" color2=\"#8b123a\"\n" +
    "                                        data1=\"ae.countAccountsWithSuccessfulSentEmail()\" data2=\"ae.countAllAccounts()\"></doughnut-chart>\n" +
    "                        <div>{{'reach' | message}}</div>\n" +
    "                    </div>\n" +
    "                    <div class=\"align-top\">\n" +
    "                        <div class=\"summarize-digit\">{{ae.getAverageSentEmailPerAccount()}}</div>\n" +
    "                        <div>{{'avgEmailsHcp' | message}}</div>\n" +
    "                    </div>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "            <div class=\"cell half section-container\">\n" +
    "                <div class=\"section-title\">{{'emailFrequency' | message}}</div>\n" +
    "                <div class=\"section-body inline-block-container\" ng-if=\"ae.mainDataReady\">\n" +
    "                    <div class=\"align-top\">\n" +
    "                        <doughnut-chart ng-style=\"ae.getDoughnutStyle()\" color1=\"#38cdb8\" color2=\"#007a7f\"\n" +
    "                                        data1=\"ae.countOpenedSentEmailFromFilteredAccount()\" data2=\"ae.countAllSuccessfulSentEmailsFromFilteredAccount()\"></doughnut-chart>\n" +
    "                        <div>{{ae.getLabelTranslation('opened')}}</div>\n" +
    "                    </div>\n" +
    "                    <div class=\"align-top\">\n" +
    "                        <report-table headers=\"ae.productFrequencyTable.headers\" data=\"ae.productFrequencyTable.data\"></report-table>\n" +
    "                    </div>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "        </div>\n" +
    "    </div>\n" +
    "\n" +
    "    <div class=\"ae-summary-section-2\">\n" +
    "        <div class=\"section-container\">\n" +
    "            <div class=\"section-title\">{{'emailFrequencyTrend' | message}}</div>\n" +
    "            <div class=\"section-body\" ng-if=\"ae.mainDataReady\">\n" +
    "                <div class=\"table structure-table\">\n" +
    "                    <div class=\"cell half align-top\">\n" +
    "                        <div class=\"chart-info-block-top\">\n" +
    "                            <div>{{ae.avgSentEmailsPerDay}}</div>\n" +
    "                            <div>{{'avgEmailsDay' | message}}</div>\n" +
    "                        </div>\n" +
    "                        <bar-chart data=\"ae.frequencyChart[ae.frequencyChartMode].data\" labels=\"ae.frequencyChart[ae.frequencyChartMode].labels\"></bar-chart>\n" +
    "                        <div class=\"chart-info-block-bottom\">\n" +
    "                            <div class=\"chart-controller inline-block-container\">\n" +
    "                                <div class=\"chart-toggle\" ng-class=\"{active:ae.frequencyChartMode === $index}\" ng-repeat=\"chart in ae.frequencyChart\"\n" +
    "                                     ng-click=\"ae.switchFrequencyChart($index)\">{{chart.name}}</div>\n" +
    "                            </div>\n" +
    "                            <div class=\"chart-legend\">\n" +
    "                                <span class=\"chart-legend-item\">\n" +
    "                                    <hr><span>{{'avgEmailsDay' | message}}</span>\n" +
    "                                </span>\n" +
    "                                <span class=\"chart-legend-item\">\n" +
    "                                    <i class=\"fa fa-square\"></i><span>{{'emails' | message}}</span>\n" +
    "                                </span>\n" +
    "                            </div>\n" +
    "                        </div>\n" +
    "                    </div>\n" +
    "                    <div class=\"cell half align-top\">\n" +
    "                        <report-table headers=\"ae.sentEmailFrequencyTable.headers\" data=\"ae.sentEmailFrequencyTable.data\"\n" +
    "                                      clickable=\"true\" row-click-handler=\"ae.goToAccountDetailView\"></report-table>\n" +
    "                    </div>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "        </div>\n" +
    "    </div>\n" +
    "\n" +
    "    <div class=\"ae-summary-section-3\">\n" +
    "        <div class=\"table structure-table\">\n" +
    "            <div class=\"cell half section-container\">\n" +
    "                <div class=\"section-title\">{{'mostSentEmails' | message}}</div>\n" +
    "                <div class=\"section-body no-padding\" ng-if=\"ae.mainDataReady\">\n" +
    "                    <report-table class=\"bottom-border\" headers=\"ae.sentDocumentBySentCountTables[ae.sentDocumentBySentCountTableMode].headers\" data=\"ae.sentDocumentBySentCountTables[ae.sentDocumentBySentCountTableMode].data\"></report-table>\n" +
    "                    <div class=\"table-info-block-bottom\">\n" +
    "                        <div class=\"table-controller\">\n" +
    "                            <div class=\"chart-toggle\" ng-class=\"{active:ae.sentDocumentBySentCountTableMode === $index}\" ng-repeat=\"table in ae.sentDocumentBySentCountTables\"\n" +
    "                                 ng-click=\"ae.sentDocumentBySentCountTableMode = $index\">{{table.type}}</div>\n" +
    "                        </div>\n" +
    "                    </div>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "            <div class=\"cell half section-container\">\n" +
    "                <div class=\"section-title\">{{'mostOpenEmails' | message}}</div>\n" +
    "                <div class=\"section-body no-padding\" ng-if=\"ae.mainDataReady\">\n" +
    "                    <report-table class=\"bottom-border\" headers=\"ae.sentDocumentByOpenRateTables[ae.sentDocumentByOpenRateTableMode].headers\" data=\"ae.sentDocumentByOpenRateTables[ae.sentDocumentByOpenRateTableMode].data\"></report-table>\n" +
    "                    <div class=\"table-info-block-bottom\">\n" +
    "                        <div class=\"table-controller\">\n" +
    "                            <div class=\"chart-toggle\" ng-class=\"{active:ae.sentDocumentByOpenRateTableMode === $index}\" ng-repeat=\"table in ae.sentDocumentByOpenRateTables\"\n" +
    "                                 ng-click=\"ae.sentDocumentByOpenRateTableMode = $index\">{{table.type}}</div>\n" +
    "                        </div>\n" +
    "                    </div>\n" +
    "                </div>\n" +
    "            </div>\n" +
    ...
    "    </div>\n" +
    "\n" +
    "    <div class=\"ae-summary-section-4\">\n" +
    "        <div class=\"table structure-table\">\n" +
    "            <div class=\"cell half section-container\">\n" +
    "                <div class=\"section-title\">{{'mostFrequentActs' | message}}</div>\n" +
    "                <div class=\"section-body no-padding\" ng-if=\"ae.mainDataReady\">\n" +
    "                    <report-table class=\"no-border\" headers=\"ae.topAccountTable.headers\" data=\"ae.topAccountTable.data\"></report-table>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "            <div class=\"cell half section-container\">\n" +
    "                <div class=\"section-title\">{{'leastFrequentActs' | message}}</div>\n" +
    "                <div class=\"section-body no-padding\" ng-if=\"ae.mainDataReady\">\n" +
    "                    <report-table class=\"no-border\" headers=\"ae.leastAccountTable.headers\" data=\"ae.leastAccountTable.data\"></report-table>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "        </div>\n" +
    "    </div>\n" +
    "</div>\n" +
    "\n" +
    "<div ng-if=\"!ae.mainView\">\n" +
    "    <account-detail-filter get-frequency-list=\"ae.getSentEmailFrequencyList()\" pre-selected-frequency=\"ae.preSelectedFrequency\" frequency-update=\"ae.sentEmailFrequencyUpdate(frequency)\"></account-detail-filter>\n" +
    "    <div class=\"ae-summary-section-5\">\n" +
    "        <div class=\"section-container\">\n" +
    "            <report-table headers=\"ae.accountDetailTable.headers\" data=\"ae.accountDetailTable.data\"\n" +
    "                          column-sortable=\"true\" custom-filter-function=\"ae.accountDetailTableFilter\"></report-table>\n" +
    "        </div>\n" +
    "    </div>\n" +
    "</div>"
  );
